...
| 9 | USER_CONFIRMATION | **Summarize for the user**: the dispatch synthesis — ticket results collected, success/failure counts, routing decision, any concerns about missing or partial results. Present this as a concise summary and wait for the user to explicitly confirm before proceeding. Do NOT send to agent — this is a controller↔user gate. If user rejects, go back to INJECT_RESEARCH with user's feedback. | **HARD GATE — must have user approval** |
