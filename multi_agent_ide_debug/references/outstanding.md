...
## 51. Discovery orchestrator skipped agent dispatch — collector synthesized hallucinated findings

**Problem:** Goal `ak:01KN3CAJF6C1TEJG13T68893XG` — the discovery orchestrator saw `routeBackCount: 1` and concluded agents had already been dispatched. Instead of dispatching discovery agents, it called `call_agent` back to the root orchestrator asking "did discovery agents already run?" and then went straight to the discovery collector. The collector produced a detailed consolidation of "prior discovery findings" that were never actually generated by discovery agents reading the codebase.

**Root cause:** The discovery orchestrator's execution history showed it was on a second invocation (route-back), so it assumed prior agents had run and results existed. No mechanism exists to verify whether child agents were actually dispatched and completed. The collector had no way to verify the provenance of the findings it consolidated.

**Workaround:** None — the hallucinated findings happened to be plausible because the goal was well-specified enough for the LLM to guess correctly. Planning phase is proceeding on potentially inaccurate discovery.

**Fix needed:**
1. **Collector/dispatcher checklists**: Add a checklist ACTION step requiring the controller to verify that child agents actually dispatched before approving a collector's ADVANCE_PHASE. Check the workflow graph for discovery-agent/planning-agent/ticket-agent child nodes under the dispatch node.
2. **Agent call graph in call_controller**: Include the current agent call graph (which nodes exist, their statuses) in the `call_controller` request context so the controller can see whether agents actually ran during the justification review.

## 52. Discovery orchestrator calls root orchestrator via call_agent — unnecessary inter-agent communication

**Problem:** Goal `ak:01KN3CAJF6C1TEJG13T68893XG` — the discovery orchestrator used `call_agent` targeting the root orchestrator (`targetAgentType: orchestrator`) to ask about workflow state, instead of using its own execution history or proceeding with its defined role (dispatching agents).

**Root cause:** The discovery orchestrator saw conflicting signals — it was invoked with `routeBackCount: 1` but had no memory of prior agent dispatch. It used `call_agent` as an escape hatch to query the parent, creating an `AGENT_TO_AGENT_CONVERSATION` node. This is outside the expected topology (discovery-orchestrator should only dispatch to discovery-agent-dispatch or discovery-collector).

**Workaround:** The call_agent permission was approved (ALLOW_ALWAYS) and the conversation completed. The workflow continued, but it burned tokens and time on an unnecessary round-trip.

**Fix needed:** Consider whether `call_agent` from a sub-orchestrator back to a parent orchestrator should be blocked or warned against. The execution history and workflow position context should provide enough state for the agent to make routing decisions without querying upstream agents.
