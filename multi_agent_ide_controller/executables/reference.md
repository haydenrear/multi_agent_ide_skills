...
| `permissions.py` | List all pending permissions with full tool name and rawInput. Use `--resolve` to approve all after inspection, `--option REJECT_ONCE` to reject. `--detail-timeout N` (default 6s) retries the tool-call detail fetch before blind-approving — handles the ACP buffer delay on first permissions. |
...
| `register_propagator.py` | List, check, or register propagators. `--list <layerId>` shows propagators on a layer, `--list-all` shows all layers with counts, `--check-missing` finds layers with 0 propagators, `--register <layerId> --stage ACTION_REQUEST` registers a new AI_TEXT propagator. Note: manual registration currently broken (outstanding #29). |
